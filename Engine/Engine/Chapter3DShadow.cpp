...
#pragma region Init Shader
	...
	//3d
	Shader* cubeShaderMapping = new Shader("assets/Material/Shader/cube_shader_mapping.vert", "assets/Material/Shader/cube_shader_mapping.frag");
	Shader* pointShadowsDepthShader = new Shader("assets/Material/Shader/point_shadows_depth.vert", "assets/Material/Shader/point_shadows_depth.frag", "assets/Material/Shader/point_shadows_depth.geom");

	glBindBufferBase(GL_UNIFORM_BUFFER, 1, uboMatrices);


	cubeShaderMapping->Use();
	cubeShaderMapping->SetUniform1i("diffuseTexture", 0);
	cubeShaderMapping->SetUniform1i("depthMap", 1);
...
#pragma region Render Loop
	while (!glfwWindowShouldClose(window)) {
		...
		for (GLuint i = 0; i < 6; ++i){
			pointShadowsDepthShader->SetUniformMatrix4fv(("shadowMatrices[" + std::to_string(i) + "]").c_str(), shadowTransforms[i]);
		}
		pointShadowsDepthShader->SetUniform1f("far_plane", far);
		pointShadowsDepthShader->SetUniform3fv("lightPos", lightPos);
		RenderScene(*pointShadowsDepthShader);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
// --------------------------------------------------------------
		cubeShaderMapping->Use();
		// set light uniforms
		glm::mat4 projection = glm::perspective(mycamera->zoom, (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
		glm::mat4 view = mycamera->GetViewMatrix();
		cubeShaderMapping->SetUniformMatrix4fv("projection", projection);
		cubeShaderMapping->SetUniformMatrix4fv("view", view);

		cubeShaderMapping->SetUniform3fv("lightPos", lightPos);
		cubeShaderMapping->SetUniform3fv("viewPos", mycamera->position);

		cubeShaderMapping->SetUniform1i("shadows", true);
		cubeShaderMapping->SetUniform1f("far_plane", far);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, leather->TexBuffer);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_CUBE_MAP, depthCubemap);
		RenderScene(*cubeShaderMapping);
